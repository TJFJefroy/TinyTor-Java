 ...
 *
 * Field elements are written as an array of signed, 64-bit limbs (an array of
 * longs), least significant first. The value of the field element is: x[0] +
 * 2^26·x[1] + x^51·x[2] + 2^77·x[3] + 2^102·x[4] + 2^128·x[5] + x^153·x[6] +
 * 2^179·x[7] + 2^204·x[8] + 2^230·x[9]
 *
 * i.e. the limbs are 26, 25, 26, 25, ... bits wide.
 *
 * Example Usage:
 *
 * Alice: byte[] privateKeyA = Curve25519.GeneratePrivateKey(); byte[]
 * publicKeyA = Curve25519.x25519PublicFromPrivate(privateKeyA); Bob: byte[]
 * privateKeyB = Curve25519.GeneratePrivateKey(); byte[] publicKeyB =
 * Curve25519.x25519PublicFromPrivate(privateKeyB);
 *
 * Alice sends publicKeyA to Bob and Bob sends publicKeyB to Alice. Alice:
 * byte[] sharedSecretA = Curve25519.x25519(privateKeyA, publicKeyB); Bob:
 * byte[] sharedSecretB = Curve25519.x25519(privateKeyB, publicKeyA); such that
...
public final class ED25519 {
	...
	/**
	 * Find the difference of two numbers: output = in1 - in2 (note the order of the
	 * arguments!).
	 */
	...
	/**
	 * Find the difference of two numbers: output = in - output (note the order of
	 * the arguments!).
	 */
	...
	private static void product(long[] out, long[] in2, long[] in) {
		...
		out[6] = 2 * (in2[3] * in[3] + in2[1] * in[5] + in2[5] * in[1]) + in2[2] * in[4] + in2[4] * in[2]
				+ in2[0] * in[6] + in2[6] * in[0];
		out[7] = in2[3] * in[4] + in2[4] * in[3] + in2[2] * in[5] + in2[5] * in[2] + in2[1] * in[6] + in2[6] * in[1]
				+ in2[0] * in[7] + in2[7] * in[0];
		out[8] = in2[4] * in[4] + 2 * (in2[3] * in[5] + in2[5] * in[3] + in2[1] * in[7] + in2[7] * in[1])
				+ in2[2] * in[6] + in2[6] * in[2] + in2[0] * in[8] + in2[8] * in[0];
		out[9] = in2[4] * in[5] + in2[5] * in[4] + in2[3] * in[6] + in2[6] * in[3] + in2[2] * in[7] + in2[7] * in[2]
				+ in2[1] * in[8] + in2[8] * in[1] + in2[0] * in[9] + in2[9] * in[0];
		out[10] = 2 * (in2[5] * in[5] + in2[3] * in[7] + in2[7] * in[3] + in2[1] * in[9] + in2[9] * in[1])
				+ in2[4] * in[6] + in2[6] * in[4] + in2[2] * in[8] + in2[8] * in[2];
		out[11] = in2[5] * in[6] + in2[6] * in[5] + in2[4] * in[7] + in2[7] * in[4] + in2[3] * in[8] + in2[8] * in[3]
				+ in2[2] * in[9] + in2[9] * in[2];
		out[12] = in2[6] * in[6] + 2 * (in2[5] * in[7] + in2[7] * in[5] + in2[3] * in[9] + in2[9] * in[3])
	 ...
	 *
	 * On entry: |output[i]| < 14*2^54 On exit: |output[0..8]| < 280*2^54
	 */
	private static void reduceDegree(long[] output) {
		// Each of these shifts and adds ends up multiplying the value by 19.
		//
		// For output[0..8], the absolute entry value is < 14*2^54 and we add, at most,
		// 19*14*2^54 thus,
	...
	private static void reduceCoefficients(long[] output) {
		...
		for (int i = 0; i < LIMB_CNT; i += 2) {
			...
			// most, 280*2^28 in
			// the first iteration of this loop. This is added to the next limb and we can
			// approximate the
			// resulting bound of that limb by 281*2^54.
			output[i] -= over << 26;
			output[i + 1] += over;

			// For the first iteration, |output[i+1]| < 281*2^54, thus |over| < 281*2^29.
			// When this is
			// added to the next limb, the resulting bound can be approximated as 281*2^54.
			//
			// For subsequent iterations of the loop, 281*2^54 remains a conservative bound
			// and no
			// overflow occurs.
		...
		output[10] = 0;
		// Now output[1..9] are reduced, and |output[0]| < 2^26 + 19*281*2^29 so |over|
		// will be no more
		// than 2^16.
	 ...
	 *
	 * The output is reduced degree (indeed, one need only provide storage for 10
	 * limbs) and |output[i]| < 2^26.
	 */
	...
	static void square(long[] output, long[] in) {
		long[] t = new long[19];
		squareInner(t, in);
		// |t[i]| < 14*2^54 because the largest product of two limbs will be < 2^(27+27)
		// and SquareInner
	...
	static long[] expand(byte[] input) {
		...
		for (int i = 0; i < LIMB_CNT; i++) {
			output[i] = ((((long) (input[EXPAND_START[i]] & 0xff)) | ((long) (input[EXPAND_START[i] + 1] & 0xff)) << 8
	...
	/**
	 * returns 0xffffffff if a >= b and zero otherwise, where a and b are both
	 * non-negative.
	 */
	...
	/**
	 * Takes a fully reduced polynomial form number and contract it into a
	...
	static byte[] contract(long[] inputLimbs) {
		...
		for (int j = 0; j < 2; j++) {
			for (int i = 0; i < 9; i++) {
				// This calculation is a time-invariant way to make input[i] non-negative by
				// borrowing
			...
			// There's no greater limb for input[9] to borrow from, but we can multiply by
			// 19 and borrow
			...
			// After the first iteration, input[1..9] are non-negative and fit within 25 or
			// 26 bits,
			// depending on position. However, input[0] may be negative.
		}

		// The first borrow-propagation pass above ended with every limb except
		// (possibly) input[0]
		// non-negative.
		//
		// If input[0] was negative after the first pass, then it was because of a carry
		// from input[9].
		// On entry, input[9] < 2^26 so the carry was, at most, one, since (2**26-1) >>
		// 25 = 1. Thus
		// input[0] >= -19.
		//
		// In the second pass, each limb is decreased by at most one. Thus the second
		// borrow-propagation
		// pass could only have wrapped around to decrease input[0] again if the first
		// pass left
		// input[0] negative *and* input[1] through input[9] were all zero. In that
		// case, input[1] is
		// now 2^25 - 1, and this last borrow-propagation step will leave input[1]
		// non-negative.
		{
		...
		// input[9], and that
		// caused input[0] to be out-of-bounds, then input[0] was < 2^26 + 2*19, because
		// the carry was,
		// at most, two.
		//
		...
		// It still remains the case that input might be between 2^255-19 and 2^255. In
		// this case,
		// input[1..9] must take their maximum value and input[0] must be >= (2^255-19)
		// & 0x3ffffff,
		// which is 0x3ffffed.
	 ...
	 */
	private static void monty(long[] x2, long[] z2, long[] x3, long[] z3, long[] x, long[] z, long[] xprime,
			long[] zprime, long[] qmqp) {
		...
		// |xxprime[i]| < 14*2^54: the largest product of two limbs will be < 2^(27+27)
		// and fproduct
		// adds together, at most, 14 of those products. (Approximating that to 2^58
		// doesn't work out.)
		...
		// |zzz[i]| < 2^(27+17)
		// No need to call reduceDegree here: scalarProduct doesn't increase the degree
		// of its input.
	...
	/**
	 * Conditionally swap two reduced-form limb arrays if {@code iswap} is 1, but
	 * leave them unchanged if {@code iswap} is 0. Runs in data-invariant time to
	 * avoid side-channel attacks.
	 *
	 * NOTE that this function requires that {@code iswap} be 1 or 0; other values
	 * give wrong results. Also, the two limb arrays must be in reduced-coefficient,
	 * reduced-degree form: the values in a[10..19] or b[10..19] aren't swapped, and
	 * all all values in a[0..9],b[0..9] must have magnitude less than
	 * Integer.MAX_VALUE.
	 */
	...
	/**
	 * Conditionally copies a reduced-form limb arrays {@code b} into {@code a} if
	 * {@code icopy} is 1, but leave {@code a} unchanged if 'iswap' is 0. Runs in
	 * data-invariant time to avoid side-channel attacks.
	 *
	 * NOTE that this function requires that {@code icopy} be 1 or 0; other values
	 * give wrong results. Also, the two limb arrays must be in reduced-coefficient,
	 * reduced-degree form: the values in a[10..19] or b[10..19] aren't swapped, and
	 * all all values in a[0..9],b[0..9] must have magnitude less than
	 * Integer.MAX_VALUE.
	 ...
	 *
	 * @param resultx the x projective coordinate of the resulting curve point
	 *                (short form)
	 * @param resultz the z projective coordinate of the resulting curve point
	 *                (short form)
	 ...
	 *
	 * Shamelessly copied from agl's code which was shamelessly copied from djb's
	 * code. Only the comment format and the variable namings are different from
	 * those.
	 */
	 ...
	 *
	 * Note from BoringSSL: All X25519 implementations should decode scalars
	 * correctly (see https://tools.ietf.org/html/rfc7748#section-5). However, if an
	 * implementation doesn't then it might interoperate with random keys a fraction
	 * of the time because they'll, randomly, happen to be correctly formed.
	 *
	 ...
	 *
	 * This does not affect security because, although we're throwing away entropy,
	 * a valid implementation of x25519 should throw away the exact same bits
	 * anyway.
	 */
	...
	/**
	 * Returns the 32-byte shared key (i.e., privateKey·peersPublicValue on the
	 * curve).
	 *
	 * @param privateKey       32-byte private key
	 * @param peersPublicValue 32-byte public value
	 * @return the 32-byte shared key
	 * @throws IllegalArgumentException when either {@code privateKey} or
	 *                                  {@code peersPublicValue} is not 32 bytes.
	 */
	 ...
	 * {@code privateKey} (i.e., {@code privateKey}·[9] on the curve).
	 *
	 * @param privateKey 32-byte private key
